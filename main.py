...
def create_pdf_from_pics(path, name):
    ...
    pdf_file = os.path.join(PDF_DIRCTORY_NAME,name + file_start+'.pdf')
    if len(images) > 1:
...
if __name__ == '__main__':
    ...
    for t in capture_time:
        scheduler.add_job(capture_screen, 'cron', 
                            hour=int(t.split(':')[0]), 
    ...
    scheduler.add_job(create_pdf_from_pics, 'cron',
                        hour=int(create_pdf_time.split(':')[0]), 
                        minute=int(create_pdf_time.split(':')[1]), 
                        second=int(create_pdf_time.split(':')[2]),
                        args=(picture_path, name,)
                    )
